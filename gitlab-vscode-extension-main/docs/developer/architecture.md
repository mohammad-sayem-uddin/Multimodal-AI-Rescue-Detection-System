...
The GitLab for VS Code extension provides a "glue" layer between the [VS Code Extension API](https://code.visualstudio.com/api/references/vscode-api) and the GitLab instance API ([REST](https://docs.gitlab.com/api/api_resources/), [GraphQL](https://docs.gitlab.com/api/graphql/)). The extension code aims to connect VS Code editor with GitLab functionality as efficiently as possible. The less code and abstractions, the better.

## Environments

The extension runs in two different environments:
...
This section describes code specific to the Desktop version. The key things to keep in mind are the desktop code:
...
graph LR;
subgraph extension
A[Extension] --> B[issuable_controller]
end
subgraph webview
