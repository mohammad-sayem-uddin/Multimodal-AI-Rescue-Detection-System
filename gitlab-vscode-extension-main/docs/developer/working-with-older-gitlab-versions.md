...
When you add an account to the extension, the instance URL is used as the key to store the authentication token. To have multiple accounts corresponding to different GitLab versions, you need a unique URL for each GitLab instance.
...
### Connect to the GDK with your local extension

Assuming you have completed the basic setup:

1. Open VS Code with the GitLab extension installed.
