...
export const lspBasePath = path.join(root, 'node_modules/@gitlab-org/gitlab-lsp/out');
...
const copyLanguageServerAssets = env => {
  ...
  const copyPromises = platformAssets.map(asset =>
    copy(asset, path.join(`dist-${env}/assets/language-server`, asset.replace(lspBasePath, ''))),
  );
