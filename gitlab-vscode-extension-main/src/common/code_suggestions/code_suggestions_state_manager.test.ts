...
describe('Code suggestions state manager', () => {
  ...
  describe('init', () => {
    it('if a policy initialization fails, the init does not crash', async () => {
      ...
      const mockFeatureStateProvider = createFakePartial<LanguageServerFeatureStateProvider>({
        setStates: jest.fn(),
      });
  ...
  describe('visible state', () => {
    ...
    beforeEach(async () => {
      ...
      // Create a proper mock for the feature state provider
      const mockFeatureStateProvider = createFakePartial<LanguageServerFeatureStateProvider>({
        setStates: jest.fn(),
      });
