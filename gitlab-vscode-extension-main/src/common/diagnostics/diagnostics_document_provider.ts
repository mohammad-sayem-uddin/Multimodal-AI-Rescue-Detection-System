...
/**
 * Generates the diagnostics markdown content from a diagnostics service.
 * This function can be used both for displaying diagnostics and exporting them.
 * @param service The diagnostics service to get sections from
 * @returns The formatted markdown string
 */
export function generateDiagnosticsMarkdown(service: DiagnosticsService): string {
  const documentTitle = '# GitLab Workflow Diagnostics';
  const formattedSections = service
    .getSections()
...
export class DiagnosticsDocumentProvider implements vscode.TextDocumentContentProvider {
  ...
  provideTextDocumentContent(uri: vscode.Uri): vscode.ProviderResult<string> {
    ...
    try {
      return generateDiagnosticsMarkdown(this.#service);
    } catch (e) {
