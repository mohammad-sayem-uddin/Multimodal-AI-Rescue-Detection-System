...
import { ENABLE_SANDBOX, DISABLE_SANDBOX } from '../constants';
import { COMMAND_TOGGLE_SANDBOX } from '../../sandbox/commands/toggle_sandbox';
...
describe('show duo quick pick menu command', () => {
  ...
  describe('sandbox status item', () => {
    const sandboxItemMock = { label: '$(pass) GitLab Duo Agent Sandboxing: Enabled' };

    beforeEach(() => {
      jest.spyOn(utils, 'generateSandboxToggleItem').mockReturnValue(undefined);
    });

    it('does not show sandbox item when no languageServerFeatureStateProvider is given', () => {
      expect(findItemByLabel(sandboxItemMock.label)).toBeUndefined();
    });

    it('shows sandbox item in the status section when provider returns an item', async () => {
      jest.spyOn(utils, 'generateSandboxStatusItem').mockReturnValue(sandboxItemMock);

      await showDuoQuickPickMenu({
        stateManager: mockStateManager,
        languageServerFeatureStateProvider: {} as never,
      });

      expect(findItemByLabel(sandboxItemMock.label)).toBeDefined();
    });

    it('does not show sandbox item when provider returns undefined (LS not yet reporting)', async () => {
      jest.spyOn(utils, 'generateSandboxStatusItem').mockReturnValue(undefined);
    ...
    it('does not show sandbox item when account is missing', async () => {
      jest.spyOn(utils, 'generateSandboxStatusItem').mockReturnValue(sandboxItemMock);
    ...
    it('toggles sandbox when the toggle item is selected', async () => {
      const toggleItem = { label: ENABLE_SANDBOX };
      jest.spyOn(utils, 'generateSandboxToggleItem').mockReturnValue(toggleItem);
      ...
      const selectionCallback = jest.mocked(mockQuickPick.onDidChangeSelection).mock.calls[0][0];
      await selectionCallback([toggleItem]);

      expect(vscode.commands.executeCommand).toHaveBeenCalledWith(COMMAND_TOGGLE_SANDBOX);
      expect(mockQuickPick.hide).toHaveBeenCalled();
    });

    it('does not show sandbox toggle item when provider returns undefined', async () => {
      jest.spyOn(utils, 'generateSandboxToggleItem').mockReturnValue(undefined);
      ...
      expect(findItemByLabel(ENABLE_SANDBOX)).toBeUndefined();
      expect(findItemByLabel(DISABLE_SANDBOX)).toBeUndefined();
    });
  });
