...
const toggleSandbox = () => vscode.commands.executeCommand(COMMAND_TOGGLE_SANDBOX);
...
export const showDuoQuickPickMenu = async ({
  ...
  const sandboxStatusItem = languageServerFeatureStateProvider
    ? generateSandboxStatusItem(languageServerFeatureStateProvider)
    : undefined;
  const sandboxToggleItem = languageServerFeatureStateProvider
    ? generateSandboxToggleItem(languageServerFeatureStateProvider)
    : undefined;
  if (!isMissingAccount) {
    statusItems.push(codeSuggestionsStatusItem, duoChatStatusItem);
    if (sandboxStatusItem) statusItems.push(sandboxStatusItem);
    toggleItems.push(
      ...[
        ...(codeSuggestionsLanguageToggleItem ? [codeSuggestionsLanguageToggleItem] : []),
        codeSuggestionsToggleItem,
        ...(sandboxToggleItem ? [sandboxToggleItem] : []),
      ],
    );
  } else {
