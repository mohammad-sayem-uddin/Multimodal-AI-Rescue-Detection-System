...
describe('Terminal Manager', () => {
  ...
  // Shared handler references
  let eventFn: RunCommandEventHandler;
  let cancelNotificationFn: (params: { workflowId: string }) => void;
  let shellIntegrationFn: (e: TerminalShellIntegrationChangeEvent) => void;
  let shellExecutionFn: (e: TerminalShellExecutionEndEvent) => void;
  let onDidCloseTerminalListener: (terminal: Terminal) => void;

  // Helper to setup standard event handlers
  const setupEventHandlers = (
    options: {
      shellExecDispose?: jest.Mock;
      shellIntDispose?: jest.Mock;
      requestDispose?: jest.Mock;
      closeTerminalDispose?: jest.Mock;
    } = {},
  ) => {
    jest.mocked(mockClient.onRequest).mockImplementation((event: string, fn) => {
      ...
      return { dispose: options.requestDispose || jest.fn() };
    });

    jest.mocked(mockClient.onNotification).mockImplementation((event: string, fn) => {
      if (event === '$/gitlab/cancelRunningCommand') {
        cancelNotificationFn = fn as (params: { workflowId: string }) => void;
      }
    ...
    jest.mocked(window.onDidChangeTerminalShellIntegration).mockImplementation(fn => {
      ...
      return { dispose: options.shellIntDispose || jest.fn() };
    });

    jest.mocked(window.onDidEndTerminalShellExecution).mockImplementation(fn => {
      shellExecutionFn = fn;
      return { dispose: options.shellExecDispose || jest.fn() };
    });

    jest.mocked(window.onDidCloseTerminal).mockImplementation(listener => {
      onDidCloseTerminalListener = listener;
      return { dispose: options.closeTerminalDispose || jest.fn() };
    });
  };
  ...
  describe('dispose', () => {
    ...
    beforeEach(async () => {
      ...
      setupEventHandlers({ requestDispose: listenerDispose, closeTerminalDispose });
  ...
  describe('$/gitlab/runCommand', () => {
    ...
    beforeEach(() => {
      ...
      setupEventHandlers({ shellExecDispose, shellIntDispose });
  ...
  describe('$/gitlab/cancelRunningCommand', () => {
    const workflowId = '1234';
    const command = 'npm';
    const args = ['run', 'test:unit'];
    const silent = false;
    ...
    it('listens to `$/gitlab/cancelRunningCommand` notification', () => {
      expect(mockClient.onNotification).toHaveBeenCalledWith(
        '$/gitlab/cancelRunningCommand',
        expect.any(Function),
      );
    });

    it('cancels a running command and returns partial output', async () => {
      mockShellIntegrationGetter.mockReturnValue(mockShellIntegration);

      // Simulate streaming output that gets interrupted
      const streamedOutput = ['line 1\n', 'line 2\n'];

      jest.mocked(mockExecution.read).mockImplementation(async function* read() {
        yield streamedOutput[0];
        yield streamedOutput[1];
      });

      const resultPromise = eventFn({ workflowId, command, args, silent });

      await jest.advanceTimersByTimeAsync(3000);

      // Let some output be collected
      await Promise.resolve();
      await Promise.resolve();

      // Cancel the command before it completes
      cancelNotificationFn({ workflowId });

      const result = (await resultPromise) as RunCommandResult;

      expect(mockTerminal.sendText).toHaveBeenCalledWith('\x03', false); // Ctrl+C without newline
      expect(result.output).toContain('line 1');
      expect(result.exitCode).toBe(0);
    });

    it('handles cancel for non-existent workflow gracefully', () => {
      const nonExistentWorkflowId = 'does-not-exist';

      expect(() => {
        cancelNotificationFn({ workflowId: nonExistentWorkflowId });
      }).not.toThrow();
    });

    it('does not affect command if cancel arrives after completion', async () => {
      mockShellIntegrationGetter.mockReturnValue(mockShellIntegration);
      ...
      // Should not send Ctrl+C since command already completed
  ...
  describe('dispose with running commands', () => {
    const workflowId = '1234';
    const command = 'npm';
    const args = ['run', 'test:unit'];
    const silent = false;
    ...
    it('cancels running commands when disposed', async () => {
      mockShellIntegrationGetter.mockReturnValue(mockShellIntegration);
      ...
      // Dispose should cancel the running command
      terminalManager.dispose();
