...
describe('handleError', () => {
  ...
  it('includes extra action labels before Show Logs', () => {
    handleError(new Error(message), [{ label: 'Restart', run: jest.fn() }]);

    expect(showErrorMessage).toBeCalledWith(message, 'Restart', 'Show Logs');
  });

  it('calls the matched action run callback when user picks it', async () => {
    const run = jest.fn();
    showErrorMessage.mockResolvedValue('Restart');

    handleError(new Error(message), [{ label: 'Restart', run }]);
