...
describe('LanguageServerManager', () => {
  ...
  it('registers a documentSelector that covers virtual filesystem schemes', () => {
    expect(languageClientFactory.createLanguageClient).toHaveBeenCalledWith(
      context,
      expect.objectContaining({
  ...
  it('registers the command to toggle sandbox on/off', () => {
    expect(vscode.commands.registerCommand).toHaveBeenCalledWith(
      COMMAND_TOGGLE_SANDBOX,
      expect.any(Function),
    );
  });
