...
import { DuoStatusBarItem } from '../duo_status_bar_item';
import {
...
import { COMMAND_TOGGLE_SANDBOX, toggleSandboxEnabled } from '../sandbox/commands/toggle_sandbox';
import {
...
import { LogCollectorOutputChannel } from '../diagnostics/log_collector_output_channel';
...
export class LanguageServerManager implements WebviewManager, VersionProvider {
  ...
  async startLanguageServer() {
    ...
    // Create output channel with log collection for diagnostics export
    const outputChannel = new LogCollectorOutputChannel('GitLab Language Server');
    ...
    this.#client = this.#clientFactory.createLanguageClient(this.#context, {
      documentSelector: [
        ...
        { scheme: 'untitled' },
        // Virtual filesystem schemes contributed by other VS Code extensions.
        // These let Duo operate on documents whose contents live on a remote
        // server, e.g. SAP ABAP sources surfaced via the ABAP FS extension.
        { scheme: 'adt' },
        { scheme: 'semanticfs' },
      ],
    ...
    const subscriptions = [
      ...
      vscode.commands.registerCommand(COMMAND_TOGGLE_SANDBOX, () => toggleSandboxEnabled()),
      vscode.commands.registerCommand(SHOW_QUICK_PICK_MENU, () =>
        showDuoQuickPickMenu({
          stateManager,
          languageServerFeatureStateProvider: this.#languageServerFeatureStateProvider,
        }),
      ),
      vscode.commands.registerCommand(
        CODE_SUGGESTION_STREAM_ACCEPTED_COMMAND,
