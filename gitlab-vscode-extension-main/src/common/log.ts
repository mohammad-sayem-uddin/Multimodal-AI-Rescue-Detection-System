...
import { extensionLogCollector } from './diagnostics/log_collector';
...
const multilineLog = (line: string, level: LogLevel): void => {
  ...
  const fullLine = `${prefix}${padNextLines(line)}`;

  globalLog(fullLine);
  // Also collect logs for diagnostics export
  extensionLogCollector.append(fullLine);
};
