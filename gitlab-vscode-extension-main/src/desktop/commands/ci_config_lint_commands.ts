...
export const validateCiConfig: ProjectCommand = async projectInRepository => {
  ...
  if (!editor) {
    await vscode.window.showInformationMessage('GitLab: No open file.');
    return;
  }
  ...
  if (valid) {
    doNotAwait(vscode.window.showInformationMessage('GitLab: Your CI configuration is valid.'));
    return;
  }
  doNotAwait(vscode.window.showErrorMessage('GitLab: Invalid CI configuration.'));
  if (errors[0]) {
