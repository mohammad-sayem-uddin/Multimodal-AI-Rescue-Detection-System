...
import { copyLinkToActiveFile, openCommitInGitLab } from './openers';
...
describe('openers', () => {
  ...
  it('openCommitInGitLab uses project and commit', async () => {
    await openCommitInGitLab('https://gitlab.com/gitlab-org/gitlab-vscode-extension', TEST_HASH);

    expect(vscode.commands.executeCommand).toHaveBeenCalledWith(
      VS_COMMANDS.OPEN,
      vscode.Uri.parse(
        `https://gitlab.com/gitlab-org/gitlab-vscode-extension/-/commit/${TEST_HASH}`,
      ),
    );
  });
