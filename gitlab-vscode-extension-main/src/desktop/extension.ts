...
import { initializeLogging, log } from '../common/log';
...
import { InstanceFeatureFlagService } from '../common/feature_flags/instance_feature_flag_service';
...
import { DefaultAIContextManager } from '../common/chat/ai_context_manager';
import {
...
import { USER_COMMANDS as COMMON_USER_COMMANDS, VS_COMMANDS } from '../common/command_names';
...
import { exportDiagnosticsCommand } from './diagnostics/export_diagnostics_command';
...
import { openCreateNewMr } from './commands/create_mr';
...
const registerCommands = (
  ...
  const commands = {
    ...
    [USER_COMMANDS.OPEN_COMMIT_IN_GITLAB]: openers.openCommitInGitLab,
    [USER_COMMANDS.OPEN_CREATE_NEW_ISSUE]: runWithValidProject(openers.openCreateNewIssue),
    [USER_COMMANDS.OPEN_CREATE_NEW_MR]: runWithValidProject(openCreateNewMr),
    ...
    [USER_COMMANDS.REFRESH_SIDEBAR]: async () => {
      ...
      await vscode.window.withProgress({ location: { viewId: 'currentBranchInfo' } }, async () => {
...
export const activate = async (context: vscode.ExtensionContext) => {
  const outputChannel = vscode.window.createOutputChannel('GitLab');
  ...
  // Initialize OTel early; errors are non-fatal - extension works without it.
  dependencyContainer.observabilityService.initialize().catch(e => {
    log.warn('[OTel] Failed to initialize OpenTelemetry:', e);
  });
  ...
  context.subscriptions.push(
    new ProtectedBranchManager(context, gitExtensionWrapper, getProjectRepository()),
  );
  ...
  currentBranchRefresher.init(getProjectRepository());
  ...
  try {
    ...
    await instanceFeatureFlagService.init();
    context.subscriptions.push(instanceFeatureFlagService);
    ...
    handleError(new UserFriendlyError('GitLab Language Server failed to start', error), [
      {
        label: 'Reload Extension',
        run: () => vscode.commands.executeCommand(VS_COMMANDS.RELOAD_WINDOW),
      },
    ]);
  }
  ...
  registerCommands(context, languageServerManager, getWorkspaceAccountManager(), repositoryClient);

  // Export diagnostics is desktop-only (requires Node.js archiver)
  context.subscriptions.push(
    vscode.commands.registerCommand(
      COMMON_USER_COMMANDS.EXPORT_DIAGNOSTICS,
      exportDiagnosticsCommand(diagnosticsService),
    ),
  );
