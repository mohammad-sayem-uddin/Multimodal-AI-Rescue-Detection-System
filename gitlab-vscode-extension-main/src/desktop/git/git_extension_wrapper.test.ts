...
describe('GitExtensionWrapper', () => {
  ...
  describe('repositories', () => {
    ...
    it('returns git repositories when the extension is enabled', async () => {
      ...
      expect(wrapper.gitRepositories).toHaveLength(1);

      const [gitRepository] = wrapper.gitRepositories;

      expect(gitRepository).toBeInstanceOf(GitRepositoryImpl);
      expect(gitRepository.rawRepository).toBe(fakeRepository);
    });
