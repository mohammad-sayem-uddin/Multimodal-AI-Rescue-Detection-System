...
export interface GitRepository extends vscode.Disposable {
  rootFsPath: string;
  remotes: GitRemote[];
  hasSameRootAs(repository: Repository): boolean;
  readonly rawRepository: Repository;
  readonly onBranchDidChange: vscode.Event<string>;
}
...
export class GitRepositoryImpl implements GitRepository {
  #onBranchDidChangeEmitter = new vscode.EventEmitter<string>();

  onBranchDidChange = this.#onBranchDidChangeEmitter.event;

  #changeStateListener: vscode.Disposable;
  ...
  constructor(rawRepository: Repository) {
    ...
    this.#currentBranch = rawRepository.state.HEAD?.name ?? '';
    this.#changeStateListener = rawRepository.state.onDidChange(() => {
      const newBranchName = rawRepository.state.HEAD?.name ?? '';
      if (newBranchName !== this.#currentBranch) {
        this.#currentBranch = newBranchName;
        this.#onBranchDidChangeEmitter.fire(newBranchName);
      }
    });
  }
  ...
  dispose() {
    this.#onBranchDidChangeEmitter.dispose();
    this.#changeStateListener.dispose();
  }
}
