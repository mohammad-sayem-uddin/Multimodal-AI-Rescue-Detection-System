...
const icon = (name: IconName, color?: string) =>
  new vscode.ThemeIcon(name, color ? new vscode.ThemeColor(color) : undefined);

const syncingIcon = icon('sync~spin');
...
export const getJobMetadata = (job: RestJob, isBusy: boolean = false): CiStatusMetadata => {
  ...
  if (isBusy && data.contextAction === 'cancellable') {
    data = STATUS_METADATA.canceling;
  } else if (isBusy) {
    data = { ...data, icon: syncingIcon };
  }
...
export const getPipelineMetadata = (
  ...
  let data = STATUS_METADATA[pipeline.status] || UNKNOWN_STATUS;
  if (isBusy && data.contextAction === 'cancellable') {
    data = STATUS_METADATA.canceling;
  } else if (isBusy) {
    data = { ...data, icon: syncingIcon };
