...
import { BranchProtection } from '../api/git';
...
describe('ProtectedBranchProvider', () => {
  ...
  beforeEach(() => {
    workspaceState = createExtensionContext().workspaceState;
    provider = new ProtectedBranchProvider(workspaceState, uri, projectInRepository);
  ...
  it('uses the Memento as a cache', async () => {
    const { promise } = promiseFromEvent(provider.onDidChangeBranchProtection);

    const cacheKey = `branch-rules:${uri.toString()}`;
    const fakeValue: BranchProtection[] = [
      {
        remote: 'fakeRemote',
        rules: [{ include: ['fakeBranch'] }],
      },
    ];
    await workspaceState.update(cacheKey, fakeValue);
    const value = provider.provideBranchProtection();
    expect(value).toBe(fakeValue);

    await promise;

    const newValue = provider.provideBranchProtection();
    expect(newValue).not.toBe(fakeValue);
  });
  ...
  it('calls the API', async () => {
    ...
    expect(workspaceState.keys()).toHaveLength(0);
    ...
    expect(workspaceState.keys()).toHaveLength(1);
  });
