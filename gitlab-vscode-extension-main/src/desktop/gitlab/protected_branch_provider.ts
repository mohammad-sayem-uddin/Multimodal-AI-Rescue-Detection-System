...
export class ProtectedBranchProvider implements BranchProtectionProvider, vscode.Disposable {
  ...
  constructor(workspaceState: vscode.Memento, uri: vscode.Uri, project: ProjectInRepository) {
    this.#workspaceState = workspaceState;
    this.#uri = uri;
    this.#cacheKey = `branch-rules:${uri.toString()}`;
  ...
  provideBranchProtection(): BranchProtection[] {
    if (vscode.workspace.getConfiguration('gitlab').branchProtection === false) {
      doNotAwait(this.#workspaceState.update(this.#cacheKey, undefined));
      return [];
    }
    ...
    if (this.#rules === undefined || this.#accessLevel === undefined) {
      ...
      return this.#workspaceState.get<BranchProtection[]>(this.#cacheKey, []);
    }
    ...
    doNotAwait(this.#workspaceState.update(this.#cacheKey, result));
