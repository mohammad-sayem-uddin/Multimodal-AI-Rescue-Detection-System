...
async function getSourceNamespace(
  issuable: RestIssuable,
  gitlabService: GitLabService,
): Promise<string | undefined> {
  if (!isMr(issuable)) return undefined;
  if (issuable.source_project_id === issuable.target_project_id) return undefined;
  try {
    const sourceProject = await gitlabService.fetchFromApi<RestProject>({
      type: 'rest',
      method: 'GET',
      path: `/projects/${issuable.source_project_id}`,
    });
    return sourceProject.path_with_namespace;
  } catch {
    return undefined;
  }
}
...
async function initPanelIfActive(
  ...
  const [discussionsAndLabels, sourceNamespace] = await Promise.all([
    gitlabService.getDiscussionsAndLabelEvents(issuable).catch(e => {
      handleError(e);
      return [];
    }),
    getSourceNamespace(issuable, gitlabService),
  ]);

  const enrichedIssuable = sourceNamespace
    ? { ...issuable, source_namespace: sourceNamespace }
    : issuable;

  await waitPromise;
  await panel.webview.postMessage({
    type: 'issuableFetch',
    issuable: enrichedIssuable,
