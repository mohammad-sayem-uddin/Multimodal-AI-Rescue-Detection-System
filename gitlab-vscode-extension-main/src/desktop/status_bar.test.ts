...
describe('status_bar', () => {
  ...
  describe('MR item', () => {
    ...
    it('shows a different icon for closed or merged MR items', async () => {
      const closedMr = { ...mr, state: 'closed' } as const;
      await statusBar.refresh(createBranchInfo({ mr: closedMr }));
      expect(getMrItem().text).toBe('$(git-pull-request-closed) !2000');

      const mergedMr = { ...mr, state: 'merged' } as const;
      await statusBar.refresh(createBranchInfo({ mr: mergedMr }));
      expect(getMrItem().text).toBe('$(git-merge) !2000');
    });

    it('shows a different icon for draft MR item', async () => {
      const draftMr = { ...mr, draft: true };
      await statusBar.refresh(createBranchInfo({ mr: draftMr }));
      expect(getMrItem().text).toBe('$(git-pull-request-draft) !2000');
    });

    it('shows create MR text when there is no MR', async () => {
      await statusBar.refresh(createBranchInfo());
      expect(getMrItem().text).toBe('$(git-pull-request-create) Create MR');
