...
export class StatusBar {
  ...
  updateMrItem(mr: RestMr | undefined, rootFsPath: string): void {
    ...
    if (mr) {
      ...
      this.mrStatusBarItem.command = openIssuableInWebview(mr, rootFsPath);
      this.mrStatusBarItem.text = `$(${icon}) !${mr.iid}`;
    } else {
      this.mrStatusBarItem.command = USER_COMMANDS.OPEN_CREATE_NEW_MR;
      this.mrStatusBarItem.text = '$(git-pull-request-create) Create MR';
    }
  }
