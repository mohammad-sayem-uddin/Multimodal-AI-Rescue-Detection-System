  ...
  implements vscode.TreeDataProvider<ItemModel | vscode.TreeItem>
{
  ...
  #busyPipelineIds: Set<number> = new Set();

  #busyJobIds: Set<number> = new Set();
  ...
  createPipelineItem(state: BranchState | TagState) {
    ...
    this.#pipelineItem = new PipelineRootItemModel(
      ...
      this.#busyPipelineIds.has(state.pipeline.id),
      this.#busyJobIds,
    );
  ...
  static renderInvalidState(state: InvalidState): vscode.TreeItem[] {
    if (state.isSwitchingBranch) {
      const item = new vscode.TreeItem('Switching branches...');
  ...
  async withMarkedAsBusy<T>(
    type: 'pipeline' | 'job',
    id: number,
    task: () => Promise<T>,
  ): Promise<T> {
    if (type === 'pipeline') {
      this.#busyPipelineIds.add(id);
    } else {
      this.#busyJobIds.add(id);
    }
    this.#eventEmitter.fire();
    ...
    try {
      ...
      if (type === 'pipeline') {
        this.#busyPipelineIds.delete(id);
      } else {
        this.#busyJobIds.delete(id);
      }
      this.#eventEmitter.fire();
    }
  }
