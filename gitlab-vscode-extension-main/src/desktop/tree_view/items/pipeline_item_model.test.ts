...
describe('PipelineItemModel', () => {
  describe('tree item', () => {
    ...
    it('has a busy state', () => {
      const busyItem = new PipelineItemModel(projectInRepository, pipeline, [], undefined, true);
      expect(busyItem.isBusy).toBe(true);
      const iconId = (busyItem.getTreeItem().iconPath as vscode.ThemeIcon).id;
      expect(iconId).toBe('sync~spin');
    });
  });
  ...
  describe('children', () => {
    ...
    const integrationTestJob = { ...job, id: 2, stage: 'test', name: 'integration test' };
    const packageJob = { ...job, id: 3, stage: 'package', name: 'package task' };
    ...
    it('passes jobs to each unique stage', async () => {
      ...
      expect(testJobItems.map(i => i.isBusy)).toEqual([false, false]);
      expect(packageJobItems.map(i => i.getTreeItem().label)).toEqual(['package task']);
      expect(packageJobItems.map(i => i.isBusy)).toEqual([true]);
    });
  });
});
