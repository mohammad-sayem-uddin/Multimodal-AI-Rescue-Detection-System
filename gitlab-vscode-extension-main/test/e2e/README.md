...
Utilises the WebdriverIO [VS Code Extension Testing service](https://webdriver.io/docs/extension-testing/vscode-extensions/) to test the GitLab extension using a real
instance of VSCode and `gitlab.com`. These tests will launch the latest `stable` version of VSCode with the GitLab extension installed from source.
...
To open Developer Tools in VS Code select `Help -> Toggle Developer Tools`, select the `Elements` tab to view the DOM.

Since some of the elements required to exercise the GitLab for VS Code extension are nested under multiple iframes, they can be difficult to find. To jump to the element within the DOM, select the `Select an element in the page to inspect it` (top left of the Elements toolbar) button and then select the desired element.
